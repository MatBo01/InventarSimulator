...
/**
 * Beinhaltet die Methoden für den Radix Sort Algorithmus 
 */
public class RadixSort {
	...
	/**
	 * Bekommt eine Liste an Werten übergeben und sortiert diese auf- oder absteigend
	 * 
	 * @param WaffenListe - ObservableList in der Waffen-Objekte gespeichert sind
	 * @param size - Groesse der WaffenListe
	...
	private static void vorSortierungWW(ObservableList<Waffe> WaffenListe, int size, int position, boolean richtung) {
		...
		//Array für späteres zwischenspeichern wird erstellt
		...
		if (richtung) {
			...
			//Nun werden die Werte aus der übergebenen WaffenListe an die korrekte Stelle in dem Array ablage gespeichert. Im array zaehler wird an dieser Stelle minus 1 gerechnet
			for (int i = size - 1; i >= 0; i--) {
				ablage[zaehler[(WaffenListe.get(i).getWert() / position) % 10] - 1] = WaffenListe.get(i);
				zaehler[(WaffenListe.get(i).getWert() / position) % 10]--;
			}
			//Nun wird die richtige Zahlenfolge in die WaffenListe übergeben
			for (int i = 0; i < size; i++) {
	...
	public static ObservableList<Waffe> radixSortWW(ObservableList<Waffe> WaffenListe, boolean richtung) {
		...
		//Die einzelnen Positionen der größten Zahl aus der Waffenliste werden durchgegangen
...
//-----------------------------------------------------------------------------------------------------
	...
	private static int getMaxIW(ObservableList<Item> ItemListe, int size) {
		...
		for (int i = 1; i < size; i++) {
			if (max < ItemListe.get(i).getWert()) {
				max = (int) ItemListe.get(i).getWert();
			}
		}
	...
	private static void vorSortierungIW(ObservableList<Item> ItemListe, int size, int position, boolean richtung) {
		...
		for (int i = 0; i < size; i++)
			zähler[(int) ((ItemListe.get(i).getWert() / position) % 10)]++;
		...
		if (richtung) {
			...
			for (int i = size - 1; i >= 0; i--) {
				ablage[zähler[(int) ((ItemListe.get(i).getWert() / position) % 10)] - 1] = ItemListe.get(i);
				zähler[(int) ((ItemListe.get(i).getWert() / position) % 10)]--;
			}
			for (int i = 0; i < size; i++) {
	...
	public static ObservableList<Item> radixSortIW(ObservableList<Item> ItemListe, boolean richtung) {
		...
		int max = getMaxIW(ItemListe, size);

		for (int position = 1; max / position > 0; position *= 10) {
			vorSortierungIW(ItemListe, size, position, richtung);
		}
